...
const requireTagEditor = async (c, next) => {
  ...
  if (membership.role === 'VIEWER' && membership.group.ownerUserId !== user.id) {
    throw forbidden('Viewers cannot change tags.')
  }
  c.set('tag', tag)
  await next()
}
...
tags.patch('/:id', requireTagEditor, async (c) => {
  const tag = c.get('tag')
  const form = await c.req.formData().catch(() => null)
  const data = {}
  if (typeof form?.get('name') === 'string' && form.get('name').trim())
    data.name = form.get('name').trim()
  if (typeof form?.get('color') === 'string' && form.get('color').trim())
    data.color = form.get('color').trim()
  const updated = await prisma.tag.update({ where: { id: tag.id }, data })
  return c.json({ tag: await tagToJson(updated) })
})

tags.delete('/:id', requireTagEditor, async (c) => {
  const tag = c.get('tag')
